...
Object.keys(logMap).forEach(logLevel => {
  ...
  test(`${logLevel} logs to console.${logMethod}`, () => {
    ...
    const logPrefix = expect.stringMatching(new RegExp(`${prefix}.*${logLevel.toUpperCase()}`))
...
test('defaults to process.env.LOG_LEVEL if it exists', () => {
  ...
  expect(logger.getLevel()).toBe(loglevel.levels[process.env.LOG_LEVEL.toUpperCase()])
